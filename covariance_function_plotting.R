...
load(paste0("Data/Covariance_Estimation_45_", domain, ".RData"))

output <- paste0("Plots/Real Data 45/")
...
p
ggsave(
  filename = paste0(output, "Covariance_45_", domain, ".pdf"),
  plot     = p,
  width    = plot_size,
  height   = plot_size,
  units    = "in",
...
mean_covariance_45_eucl <- data.frame(dist = colMeans(distances_45_eucl, na.rm = T), 
                                     gamma = colMeans(variograms_45_eucl, na.rm = T))

variograms_45_eucl <- melt(variograms_45_eucl)
colnames(variograms_45_eucl) <- c("Row", "Column", "Value")
variograms_45_eucl$Distance <- 
  colMeans(distances_45_eucl, na.rm = T)[variograms_45_eucl$Column]

variograms_45_eucl <-
  variograms_45_eucl[which(is.finite(variograms_45_eucl$Distance) &
                             is.finite(variograms_45_eucl$Value)),]
mean_covariance_45_eucl <-
  mean_covariance_45_eucl[which(is.finite(mean_covariance_45_eucl$dist) &
                                  is.finite(mean_covariance_45_eucl$gamma)),]
...
p
ggsave(
  filename = paste0(output, "Variogram_45_eucl_", domain, ".pdf"),
  plot     = p,
  width    = plot_size,
  height   = plot_size,
  units    = "in",
